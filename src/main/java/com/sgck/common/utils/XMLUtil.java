...
public class XMLUtil
{
	...
	public static Document stringToDocument(String xml, boolean nameSpaceAware)
	{
		...
		{
			throw new Error("Error deserializing XML type " + ex.getMessage());
		}
	 ...
	 * @throws IOException
	 */
	public static String getXmlFromASObject(ASObject as, Amf3XML amfXML) throws IOException {
		StringWriter sw = new StringWriter();
		amfXML.writeObject(sw, as, XML_MODE.MODE_FAST);
		return sw.toString();
	}
	 ...
	 */
	public static ASObject getASObjectFromXml(String xml, Amf3XML amfXML) throws DocumentException {
		if(null == xml){
			return null;
		}
		return (ASObject) amfXML.readObjectFromString(xml);
	}
}
