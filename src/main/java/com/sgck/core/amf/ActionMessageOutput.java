...
/**
 * Entry point for serializing an individual ActionMessage response message for
 * AMF, AMFX or RTMP Channels.
 * 
...
public interface ActionMessageOutput extends ObjectOutput {
	...
	/**
	 * A convenient entry point for writing out any Object for conversion to
	 * ActionScript. The Java class type of the Object will determine the
	 * corresponding ActionScript type that will be specified in the AMF stream.
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * (a) - For AMF 3 ints, taking sign into consideration, if i &lt;
	 * 0xF0000000 || i &gt; 0x0FFFFFFF, then value is promoted to Number. <br/>
	 * (b) - Dates are sent in the UTC timezone. Clients and servers must adjust
	 * time accordingly for timezones. <br/>
	 * (c) - Channel serialization configuration can be set to support legacy
	 * Collection to ActionScript Array conversion. <br/>
	 * (d) - Channel serialization configuration can be set to support legacy
	 * Map to ActionScript Array (associative) conversion. <br/>
	 * (e) - Objects are serialized using Java Bean introspection rules. Fields
	 * that are static, transient or non-public are excluded.
	 * </p>
	 * 
	 * @param object
