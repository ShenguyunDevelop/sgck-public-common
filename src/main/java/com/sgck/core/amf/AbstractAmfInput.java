 ...
 **************************************************************************/
package com.sgck.core.amf;

import java.io.DataInputStream;
import java.io.IOException;
import java.io.InputStream;

import com.sgck.common.utils.XMLUtil;
 ...
 */
public abstract class AbstractAmfInput extends AmfIO implements
		ActionMessageInput {
	/**
	 ...
	 * checks. By default, max-string-bytes is limited to 25MB for security but
	 * this can be changed via JVM option called max-string-bytes.
	 * 
	 * @param utflen
	 *            The UTF string length to check.
	 * @throws <tt>SerializationException</tt> if max-string-bytes has been
	 *         exceeded.
	 */
	protected void checkUTFLength(int utflen) {
		if (utflen > maxStringBytes) {
			// Error deserializing the string with length ''{0}'', it exceeds
			// the max-string-bytes limit of ''{1}''.
	...
	/**
	 * Internal use. Common logic for creating an object for AMF0 and AMF3. Used
	 * from Amf0Input.readObjectValue() and Amf3Input.readScriptObject(). This
	 * method is responsible for resolving class aliases, creating and
	 * registering a property proxy and instantiating an instance of the desired
	 * class. The params array is used as a holder for parameter values that are
	 * updated or created in this method, but are needed by its callers.
	 * 
	 * @param params
	 *            array of values that may be updated by this method. [0] -
	 *            String className - the name or alias of the class to create
	...
	protected Object createObjectInstance(Object[] params) {
		...
		if (aliasedClass != null) {
			...
			params[0] = className; // update the params array so that callers
									// get this change
		}
		...
		if (className != null && className.startsWith(">")) // Handle
															// [RemoteClass] (no
															// server alias)
		{
			object = createDefaultASObject(className);
		} else if (className != null && className.length() > 0
				&& (context.instantiateTypes || className.startsWith("flex."))) {
			...
			try {
				...
				// Type not found and don't want to use ASObject for the missing
				// type.
				if (!(me.getCode().startsWith(
						MessageException.CODE_SERVER_RESOURCE_UNAVAILABLE) && context.createASObjectForMissingType)) {
					throw me; // Rethrow.
				}
				// if we didn't rethrow the exception, the default ASObject will
				// be created further down.
			}

			// Type exists. Create either default instance of desiredClass or an
			// instance from a registered proxy.
			if (desiredClass != null) {
				proxy = PropertyProxyRegistry.getRegistry()
						.getProxyAndRegister(desiredClass);
				if (proxy == null) {
					object = ClassUtil.createDefaultInstance(desiredClass,
							null, true /* validate */);
				} else {
					object = proxy.createInstance(className); // Validation is
																// performed in
																// the proxy.
				}
			}
		...
		// if we still don't have an object, create an ASObject with what we
		// have for className (can be null)
		if (object == null) {
			object = createDefaultASObject(className);
		}
		...
		params[1] = proxy; // update the params array so that callers get the
							// proxy
	...
	/**
	 * Internal use. Convenience method for creating an ASObject and assigning
	 * it a type
	 * 
	 * @param type
	 *            named type for the ASObject or null
	 * @return a new instance of ASObject
	 * @exclude
	 */
	protected ASObject createDefaultASObject(String type) {
		ASObject object = (ASObject) ClassUtil.createDefaultInstance(
				ASObject.class, null, true /* validate */);
