...
public class Amf3Output extends AbstractAmfOutput implements Amf3Types {
	...
	/*
	 * public Amf3Output() { this.setDebugTrace(null); }
	 */
	...
	public void writeObject(Object o) throws IOException {
		...
		// If there is a proxy for this,write it as a custom object so the
		// default
		// behavior can be overriden.
		else if (o instanceof Enum
				&& PropertyProxyRegistry.getRegistry().getProxy(o.getClass()) == null) {
			...
			if (context.legacyMap && o instanceof Map
	...
	protected void writeAMFInt(int i) throws IOException {
		if (i >= INT28_MIN_VALUE && i <= INT28_MAX_VALUE) {
			...
			// We have to be careful when the MSB is set, as (value >> 3) will
			// sign extend.
			// We know there are only 29-bits of precision, so truncate. This
			// requires
	...
	protected void writeArrayCollection(Collection col,
		...
		if (!byReference(col)) {
			...
			if (col instanceof ArrayCollection) {
				...
				// we should modify the user's AC as that could cause
				// corruption?
			} else {
			...
			// Then wrap ArrayCollection in PropertyProxy for bean-like
			// serialization
	...
	protected void writePropertyProxy(PropertyProxy proxy, Object instance)
		...
		if (newInst != instance) {
			// We can't use writeAMFNull here I think since we already added
			// this object
			// to the object table on the server side. The player won't have any
			// way
			// of knowing we have this reference mapped to null.
			if (newInst == null) {
				throw new MessageException(
						"PropertyProxy.getInstanceToSerialize class: "
								+ proxy.getClass()
								+ " returned null for instance class: "
		...
		TraitsInfo ti = new TraitsInfo(proxy.getAlias(instance),
				proxy.isDynamic(), externalizable, propertyNames);
		...
		if (externalizable) {
			...
			throw new IOException(
					"oh,noooooooo! this class is externalizable ? but.........");
