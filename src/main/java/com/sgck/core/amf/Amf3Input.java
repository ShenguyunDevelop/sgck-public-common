 ...
 * <p>
 * This class intends to matches the Flash Player 8 C++ code in
 * avmglue/DataIO.cpp
 * </p>
 * 
...
public class Amf3Input extends AbstractAmfInput implements Amf3Types {
	...
	/*
	 * public Amf3Input(SerializationContext context) { super(context);
	 * 
	 * stringTable = new ArrayList(64); objectTable = new ArrayList(64);
	...
	protected Object readArray() throws ClassNotFoundException, IOException {
		...
		// First, look for any string based keys. If any non-ordinal indices
		// were used,
		...
		for (;;) {
			...
			if (map == null) {
				map = (HashMap) ClassUtil.createDefaultInstance(HashMap.class,
						null, true /* validate */);
				array = map;
		...
		if (map == null) {
			// Don't instantiate List/Array right away with the supplied size if
			// it is more than
			// INITIAL_ARRAY_CAPACITY in case the supplied size has been
			// tampered. This at least
			// requires the user to pass in the actual objects for the
			// List/Array to grow beyond.
			...
			if (context.legacyCollection || len > INITIAL_ARRAY_CAPACITY) {
				...
				int initialCapacity = len < INITIAL_ARRAY_CAPACITY ? len
	...
	protected void readExternalizable(String className, Object object)
		...
		if (object instanceof Externalizable) {
			...
			if (isDebug) {
				trace.startExternalizableObject(className,
						objectTable.size() - 1);
			}
	...
	protected Object readXml() throws IOException {
		...
		if ((ref & 1) == 0) {
			...
			if (0 == len) {
				xml = (String) ClassUtil.createDefaultInstance(String.class,
						null);
			} else {
				xml = readUTF(len);
			}
