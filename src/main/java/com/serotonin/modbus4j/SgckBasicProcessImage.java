...
public class SgckBasicProcessImage implements ProcessImage {
	...
	public SgckBasicProcessImage(int slaveId) {
	...
	public synchronized Number getRegister(int range, int offset, int dataType) throws IllegalDataAddressException {
		int length = DataType.getRegisterCount(DataType.getRegisterCountByMydataType(dataType));
		...
		return ModbusUtils.byte2NumberFirstByType(b, dataType);
	}
	...
	@Override
	public void registerWriter(byte[] data) {
	}
