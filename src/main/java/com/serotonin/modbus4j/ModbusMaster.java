...
abstract public class ModbusMaster extends Modbus {
	...
	protected LinkedHashMap<Integer, ModbusMasterProfie> profiles = new LinkedHashMap<Integer, ModbusMasterProfie>();

	public boolean isProfieEmpty() {
		return profiles.isEmpty();
	}

	public void addProfile(ModbusMasterProfie profile) {
		profiles.put(profile.getSlaveId(), profile);
	}

	public ModbusMasterProfie geProfile(int slaveId) {
		return profiles.get(slaveId);
	}

	public Collection<ModbusMasterProfie> geProfiles() {
		return profiles.values();
	}

	public boolean isContainProfile(Integer slaveId) {
		return profiles.containsKey(slaveId);
	}

	public void removeProfile(Integer slaveId) {
		if (isContainProfile(slaveId)) {
			ModbusMasterProfie profile = geProfile(slaveId);
			synchronized (profile) {
				profiles.remove(slaveId);
			}
		}
	}
			...
			if (errorsInResults && response.isException())
				...
				try {
					...
					throw new RuntimeException("Result conversion exception. data=" + ArrayUtils.toHexString(data)
							...
							+ functionGroup.getStartOffset() + ", functionGroup.length=" + functionGroup.getLength(),
							e);
				}
			}
		}
