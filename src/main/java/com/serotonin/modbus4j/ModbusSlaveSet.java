...
abstract public class ModbusSlaveSet extends Modbus {
	...
	public boolean isProcessImageEmpty() {
		return processImages.isEmpty();
	}
	...
	public boolean isContainProcessImage(Integer slaveId) {
		return processImages.containsKey(slaveId);
	}

	public void removeProcessImage(Integer slaveId) {
		if (isContainProcessImage(slaveId)) {
			ProcessImage processImage = getProcessImage(slaveId);
			synchronized (processImage) {
				processImages.remove(slaveId);
			}
		}
	}
	...
	/**
	 * Starts the slave. If an exception is not thrown, this method does not
	 * return, but uses the thread to execute the listening.
	 * 
	...
	abstract public void start(StartedListener startedListener) throws ModbusInitException;
}
