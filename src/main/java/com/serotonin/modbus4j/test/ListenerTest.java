...
public class ListenerTest {
	...
	public static void main(String[] args) throws Exception {
		...
		// modbusFactory.createRtuListener(processImage, 31, params, false);
		// ModbusListener listener =
		// modbusFactory.createAsciiListener(processImage, 31, params);
//		 final ModbusSlaveSet listener = modbusFactory.createTcpSlave(false);
//		 ModbusSlave listener = modbusFactory.createUdpSlave(processImage,
//		 31);

		// Add a few slave process images to the listener.

		//SerialParameters serialParameters = ModbusUtils.createModbusSerialparameter("COM6", 1, 8, 1, "01", 9600);
		final ModbusSlaveSet modbusSlaveSet = new TcpSlave(501, false);

		//modbusSlaveSet.addProcessImage(createProcessImage(0, 1));

		// When the "listener" is started it will use the current thread to run.
		// So, if an exception is not thrown
		// (and we hope it won't be), the method call will not return.
		// Therefore, we start the listener in a separate
		...
		while (true) {
			synchronized (modbusSlaveSet) {
				modbusSlaveSet.wait(5000);
			}

			for (ProcessImage processImage : modbusSlaveSet.getProcessImages()) {
				SgckBasicProcessImage bp = (SgckBasicProcessImage) processImage;
				System.out.println("new:" + bp.getRegister(RegisterRange.HOLDING_REGISTER, 0, ModbusFieldType.DOUBLE));
				// byte[] data = bp.getStore(2);
				// if (null != data) {
				// Number number =
				// ModbusUtils.byte2NumberFirstByType(bp.getStore(2), 3);
				// System.out.println("number:" + number);
				// }
				// System.out.println(processImage.getHoldingRegister(2));
				// System.out.println(processImage.getHoldingRegister(4));
				// updateProcessImage((SgckBasicProcessImage) processImage);
			}
	 ...
	 * @return
	 */
	public static SgckBasicProcessImage createProcessImage(int slaveId, final int modbusDataType) {
		...
		processImage.addListener(new ProcessImageListener() {
			...
			public void holdingRegisterWrite(int offset, short oldValue, short newValue) {
				// System.out.println("holdingRe:" + newValue);
			}

			@Override
			public void coilWrite(int offset, boolean oldValue, boolean newValue) {
				// System.out.println("coilWrite:" + newValue);
			}

		});

		processImage.getReportSlaveIdData();
		return processImage;
	}
	...
	static BasicProcessImage getModscanProcessImage(int slaveId) {
		...
		processImage.setRegister(RegisterRange.HOLDING_REGISTER, 17, DataType.FOUR_BYTE_INT_SIGNED,
				new Long(-123456789));
		processImage.setRegister(RegisterRange.HOLDING_REGISTER, 19, DataType.FOUR_BYTE_INT_SIGNED_SWAPPED,
				new Long(-123456789));
		processImage.setRegister(RegisterRange.HOLDING_REGISTER, 21, DataType.FOUR_BYTE_FLOAT, new Float(1968.1968));
		processImage.setRegister(RegisterRange.HOLDING_REGISTER, 23, DataType.EIGHT_BYTE_INT_SIGNED,
				new Long(-123456789));
		processImage.setRegister(RegisterRange.HOLDING_REGISTER, 27, DataType.EIGHT_BYTE_INT_SIGNED_SWAPPED,
				new Long(-123456789));
		...
		processImage.setRegister(RegisterRange.HOLDING_REGISTER, 12288, DataType.FOUR_BYTE_FLOAT_SWAPPED,
				new Float(1968.1968));
		processImage.setRegister(RegisterRange.HOLDING_REGISTER, 12290, DataType.FOUR_BYTE_FLOAT_SWAPPED,
				new Float(-1968.1968));
		...
		processImage.setRegister(RegisterRange.INPUT_REGISTER, 17, DataType.FOUR_BYTE_INT_UNSIGNED,
				new Long(-123456789));
		processImage.setRegister(RegisterRange.INPUT_REGISTER, 19, DataType.FOUR_BYTE_INT_UNSIGNED_SWAPPED,
				new Long(-123456789));
		processImage.setRegister(RegisterRange.INPUT_REGISTER, 21, DataType.FOUR_BYTE_FLOAT_SWAPPED,
				new Float(1968.1968));
		processImage.setRegister(RegisterRange.INPUT_REGISTER, 23, DataType.EIGHT_BYTE_INT_UNSIGNED,
				new Long(-123456789));
		processImage.setRegister(RegisterRange.INPUT_REGISTER, 27, DataType.EIGHT_BYTE_INT_UNSIGNED_SWAPPED,
				new Long(-123456789));
		processImage.setRegister(RegisterRange.INPUT_REGISTER, 31, DataType.EIGHT_BYTE_FLOAT_SWAPPED,
				new Double(1968.1968));
