...
abstract public class ModbusMessage {
    ...
    protected Number[] convertToShortNumbers(byte[] data) {
    	Number[] sdata = new Number[data.length / 2];
        for (int i = 0; i < sdata.length; i++)
            sdata[i] = ModbusUtils.toShort(data[i * 2], data[i * 2 + 1]);
        return sdata;
    }
     ...
     *@Description modbus 
     *@param data
     *@param isInvser 是否需要反过来把字节码
     *@return
     */
    protected Number[] convertToFloats(byte[] data,boolean isInvser){
    	return ModbusUtils.byte2Float(data, isInvser);
    }
    
    protected Number[] convertToLong(byte[] data,boolean isInvser){
    	return ModbusUtils.byte2Long(data,isInvser);
    }
    
    protected Number[] convertToDouble(byte[] data,boolean isInvser){
    	return ModbusUtils.byte2Double(data,isInvser);
    }
